...
namespace apn
{
	...
	struct json_io_t
	{
		...
		BOOL read()
		{
			...
			{
				hive_base->message_box(my::format(
					L"{/}を読み込み中にエラーが発生しました\n{/}",
	...
	//
	// このクラスはjson形式のコンフィグファイルの入出力を担当します。
	//
	struct json_config_io_t : json_io_t
	{
		//
		// 初期化処理を実行します。
		//
		BOOL init()
		{
			...
			path = hive_base->config_file_name =
				base.config_dir / base.path.filename().replace_extension(L".json");
			MY_TRACE_STR(hive_base->config_file_name);
